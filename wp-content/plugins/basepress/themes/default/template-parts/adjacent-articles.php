...
if ( basepress_show_adjacent_articles() ) {
	...
	if ( $bpkb_prev_article || $bpkb_next_article ) {
		...
		if ( $bpkb_prev_article ) {
			...
			echo '<a href="' . esc_url( $bpkb_prev_link ) . '">';
			echo '<span class="bpress-adjacent-title">' . esc_html( basepress_prev_article_text() ) . '</span>';
			echo '<h4>';
			if ( basepress_show_post_icon() ) {
				echo '<span class="bp-icon ' . esc_attr( $bpkb_prev_article->icon ) . '"></span>';
			}
			echo esc_html( $bpkb_prev_article->post_title ) . '</h4>';
			echo '</a>';
		}
		echo '</div>';
		echo '</div>';
		...
		if ( $bpkb_next_article ) {
			...
			echo '<a href="' . esc_url( $bpkb_next_link ) . '">';
			echo '<span class="bpress-adjacent-title">' . esc_html( basepress_next_article_text() ) . '</span>';
			echo '<h4>';
			echo esc_html( $bpkb_next_article->post_title );
			if ( basepress_show_post_icon() ) {
				echo '<span class="bp-icon ' . esc_attr( $bpkb_next_article->icon ) . '"></span>';
			}
			echo '</h4>';
			echo '</a>';
		}
