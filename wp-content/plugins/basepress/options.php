<?php

$options = array(
    'breadcrumbs_kb_name'            => 'Knowledge Base',
    'post_count_ip_exclude'          => '',
    'theme_style'                    => 'default',
    'sidebar_position'               => 'right',
    'force_sidebar_position'         => 1,
    'section_style'                  => 'list',
    'sub_section_style'              => 'list',
    'kbs_choose_text'                => 'Choose',
    'products_cols'                  => 2,
    'sections_cols'                  => 2,
    'sections_post_limit'            => 6,
    'section_post_limit'             => 20,
    'show_section_icon'              => 1,
    'show_post_icon'                 => 1,
    'show_section_post_count'        => 1,
    'show_search_suggest'            => 1,
    'min_search_suggest_screen'      => 768,
    'search_suggest_count'           => 4,
    'search_field_placeholder'       => 'Search Knowledge Base',
    'search_submit_text'             => 'Search',
    'show_search_submit'             => 1,
    'search_page_title'              => 'Found %number% results for:',
    'search_page_no_results_title'   => 'There were no results for your search.',
    'smartsearch_no_results_message' => 'No results found',
    "search_suggest_more_text"       => 'Show all results (%number%)',
    'searchbar_style'                => 1,
);
return $options;