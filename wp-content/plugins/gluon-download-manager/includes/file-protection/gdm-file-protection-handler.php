...
class gdm_File_Protection_Handler {

	public static $protected_dir_name = 'gdm-uploads';

	public static $protected_file_thumbnail = 'gdm-file-protected-thumbnail.png';

	public function __construct() {
		if(is_admin()) {
			// Listen to file protection settings update event.
			add_action('gdm_file_protection_settings_updated', array($this, 'prepare_file_protection_environment'));
	...
	public function add_custom_meta_to_specific_directory_media( $post_ID ) {
		...
		if ( $is_image_attachment && self::contains_protected_dirname($file_path) ) {
			...
			$settings = get_option( 'gdm_global_options' );
	...
	public static function get_protected_file_thumb_url(){
		return WP_GLUON_DL_MANAGER_URL . "/images/" . self::$protected_file_thumbnail;
	}
	...
	public function prepare_file_protection_environment(){
		...
		gdm_Debug::log("File protection setting updated and protection is enabled. Preparing environment for protected files if needed.", true);
	...
	public function check_and_create_file_protection_folder(){
		...
		// Check if the gdm_uploads directory exists
		...
		if ( !file_exists($htaccess_file) ) {
			try{
				...
				gdm_Debug::log("The .htaccess file was successfully created inside '".self::get_protected_dir_name() . "'.", true);
	...
	public function check_and_create_protected_file_thumbnail(){
		...
		gdm_Debug::log("The protected file thumbnail attachment post was successfully created. Attachment ID: ". $thumbnail_id, true);
	}
	...
	public function override_media_library_protected_file_thumbnail( $response, $attachment, $meta ) {
		...
		if(!gdm_File_Protection_Handler::contains_protected_dirname($dl_link)){
			return $response;
		}
		...
		if ( $attachment_post_thumbnail_id && isset($response['sizes']) && !empty($response['sizes'])) {
			if(is_array($response['sizes'])){
				...
				gdm_Debug::log("Could not override protected image file's thubmnail because the response['sizes'] is not an array.", false);
	...
	public function exclude_our_hidden_attachments_in_media_library( $query ) {
		...
		// gdm_Debug::log_array_data($existing_meta_query_val); // Debug Purpose.
...
add_action( 'plugins_loaded', function() {
	new gdm_File_Protection_Handler();
} );
