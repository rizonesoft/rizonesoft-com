...
class gdm_Protected_Download_Request_Handler
{
	...
	public static function process_enhanced_protected_download_request($download_id, $download_link){
		if (!gdm_File_Protection_Handler::contains_protected_dirname($download_link)){
		...
		$file_path = gdm_Utils_File_System_Related::get_uploaded_file_path_from_url(
			$download_link,
			 	array(
					'uploads',
					gdm_File_Protection_Handler::get_protected_dir_name()
				)
			);
	...
	public static function gdm_download_using_fopen($file_path, $chunk_blocks = 8, $session_close = false) {
		...
		$mimetype = gdm_Utils_File_System_Related::dl_file_type($file_path);
		header("Content-Type: " . $mimetype);
		header("Content-Disposition: attachment; filename=\"$file_name\"");
		header("Content-Transfer-Encoding: binary");
		header("Content-Length: " . gdm_Utils_File_System_Related::dl_filesize($file_path));
