...
function gdm_generate_fancy2_popular_downloads_display_output( $get_posts, $args ) {
	wp_enqueue_style( 'gdm_generate_fancy2_popular_downloads_display_styles', WP_GLUON_DL_MANAGER_URL . '/includes/templates/fancy2/gdm-fancy-2-styles.css' , array(), WP_GLUON_DL_MANAGER_VERSION );
    $output	 = "";

    $count = 1;
    //$output .= '<ul class="gdm_fancy2_category_items">';
    foreach ( $get_posts as $item ) {
	$opts		 = $args;
	$opts[ 'id' ]	 = $item->ID;
	$output		 .= gdm_generate_fancy2_display_output( $opts );
...
function gdm_generate_fancy2_latest_downloads_display_output( $get_posts, $args ) {
	wp_enqueue_style( 'gdm_generate_fancy2_latest_downloads_display_styles', WP_GLUON_DL_MANAGER_URL . '/includes/templates/fancy2/gdm-fancy-2-styles.css' , array(), WP_GLUON_DL_MANAGER_VERSION );
    $output	 = "";

    $count = 1;
    //$output .= '<ul class="gdm_fancy2_category_items">';
    foreach ( $get_posts as $item ) {
	$output .= gdm_generate_fancy2_display_output(
...
function gdm_generate_fancy2_category_display_output( $get_posts, $args ) {
	wp_enqueue_style( 'gdm_generate_fancy2_category_display_styles', WP_GLUON_DL_MANAGER_URL . '/includes/templates/fancy2/gdm-fancy-2-styles.css' , array(), WP_GLUON_DL_MANAGER_VERSION );
    $output	 = "";
    ...
    foreach ( $get_posts as $item ) {
        ...
        $output .= gdm_load_template(2, $tpl_data, false);
...
function gdm_generate_fancy2_display_output( $args ) {
    ...
    // Get item thumbnail
    $thumbnail_alt = apply_filters ( 'gdm_download_fancy_2_thumbnail_alt', $item_title, $id );//Trigger a filter for the thumbnail alt
    ...
    $isset_download_thumbnail	 = apply_filters( 'gdm_download_fancy_2_thumbnail', $isset_download_thumbnail, $args ); //Apply filter so it can be customized.
    ...
    $output	 .= '<div class="gdm_fancy2_item ' . esc_attr($css_class) . '">';
    ...
    $output = apply_filters( 'gdm_generate_fancy2_display_output_html', $output, $args );
