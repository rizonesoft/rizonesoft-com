...
function gdm_generate_fancy3_popular_downloads_display_output( $get_posts, $args ) {
	...
	$opts = $args;
	$opts[ 'id' ] = $item->ID;
	$output .= gdm_generate_fancy3_display_output( $opts );
    }
...
function gdm_generate_fancy3_latest_downloads_display_output( $get_posts, $args ) {
    ...
    foreach ( $get_posts as $item ) {
	$output .= gdm_generate_fancy3_display_output(
...
function gdm_generate_fancy3_category_display_output( $get_posts, $args ) {
    ...
    foreach ( $get_posts as $item ) {
        ...
        $output .= gdm_load_template(3, $tpl_data, false);
...
function gdm_generate_fancy3_display_output( $args ) {
    ...
    $output .= '<div class="gdm_fancy3_download_item ' . esc_attr($css_class) . '">';
    ...
    $download_details_link_code = apply_filters( 'gdm_fancy3_view_details_link_code_html', $download_details_link_code );
