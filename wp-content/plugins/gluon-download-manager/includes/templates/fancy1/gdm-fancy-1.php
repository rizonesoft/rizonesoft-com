...
function gdm_generate_fancy1_popular_downloads_display_output( $get_posts, $args ) {
	...
	$opts		 = $args;
	$opts[ 'id' ]	 = $item->ID;
	$output		 .= gdm_generate_fancy1_display_output( $opts );
    }
...
function gdm_generate_fancy1_latest_downloads_display_output( $get_posts, $args ) {
    ...
    foreach ( $get_posts as $item ) {
	$output .= gdm_generate_fancy1_display_output(
...
function gdm_generate_fancy1_category_display_output( $get_posts, $args ) {
    ...
    foreach ( $get_posts as $item ) {
        ...
        $output .= gdm_load_template(1, $tpl_data, false);
...
function gdm_generate_fancy1_display_output( $args ) {
    ...
    // $output = '<pre>'. print_r(get_post_meta($id, 'gdm_download_button_text'),  true) .'</pre>';
    ...
    // Get CPT thumbnail
    $thumbnail_alt = apply_filters ( 'gdm_download_fancy_1_thumbnail_alt', $item_title, $id );//Trigger a filter for the thumbnail alt
    ...
    $isset_download_thumbnail	 = apply_filters( 'gdm_download_fancy_1_thumbnail', $isset_download_thumbnail, $args ); //Apply filter so it can be customized.
    ...
    $output	 .= '<div class="gdm_download_item ' . esc_attr($css_class) . '">';
    ...
    $params = array( 'id' => $id );
    $output .= apply_filters( 'gdm_fancy1_below_download_description', '', $params);
    ...
    $output = apply_filters( 'gdm_generate_fancy1_display_output_html', $output, $args );
