...
class gdm_SWPM_Integration {
	...
	public function __construct() {
		...
		if ( is_plugin_active( 'simple-membership/simple-wp-membership.php' ) ) {
			add_action( 'plugins_loaded', array( $this, 'setup_swpm_integration' ) );
			add_action( 'gdm_file_protection_settings_updated', array( $this, 'update_swpm_integration_settings' ) );
			add_action( 'gdm_after_file_protection_settings_fields', array( $this, 'show_swpm_integration_settings' ) );
		}
	}
	...
	public function setup_swpm_integration() {
		...
		// Check if SWPM access control feature enabled for GDM downloads.
		...
		if ( $is_swpm_access_control_enabled ) {
			...
			add_action( 'gdm_process_download_request', array( $this, 'check_gdm_download_process' ), 5, 2 );
			add_action( 'gdm_sf_process_download_request', array( $this, 'check_gdm_download_process' ), 5, 2 );

			// Override download button html for unauthorized visitor.
			add_filter( 'gdm_download_button_code_html', array( $this, 'disable_gdm_download_button' ) );
		}
	}
	...
	public function show_swpm_integration_settings() {
		...
		$output = '<tr>';
		$output .= '<th scope="row">' . __( 'Enable SWPM Access Control', 'gluon-download-manager' ) . '</th>';
		$output .= '<td>';
		...
		$output .= '<p class="description">' . __( 'Check this box to enable the access control feature of the Simple Membership plugin.', 'gluon-download-manager' ) . '</p>';
		$output .= '</td>';
		$output .= '</tr>';
		echo $output;
	}

	public function check_gdm_download_process( $dl_id, $dl_link ) {
	...
	public function is_download_permitted( $dl_id ) {
		...
		remove_filter( 'swpm_restricted_post_msg_older_post', array(
			$this,
			"override_restricted_post_msg_older_post"
		) );
		...
		if ( ! $is_permitted ) {
			...
			if ( method_exists( $swpm_access_control, 'get_lastError' ) ) {
				...
				$this->error_msg = __( 'Error! Your account does not have permission to access this download item.', 'gluon-download-manager' );
			}
		}
	 ...
	 */
	public function disable_gdm_download_button( $btn_html ) {
		$dom = new DOMDocument();
...
// Only instantiate after WordPress is fully loaded to prevent activation errors
add_action( 'plugins_loaded', function() {
	new gdm_SWPM_Integration();
} );
