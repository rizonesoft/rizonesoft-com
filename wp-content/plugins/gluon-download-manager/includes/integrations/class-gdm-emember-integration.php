...
class gdm_eMember_Integration {
	...
	public function __construct() {
		...
		if ( is_plugin_active( 'wp-eMember/wp_eMember.php' ) ) {
			add_action( 'plugins_loaded', array( $this, 'setup_eMember_integration' ) );
			add_action( 'gdm_file_protection_settings_updated', array( $this, 'update_eMember_integration_settings' ) );
			add_action( 'gdm_after_file_protection_settings_fields', array(
				$this,
				'show_eMember_integration_settings'
			) );
		}
	}

	public function setup_eMember_integration() {
		...
		if ( $is_swpm_access_control_enabled ) {
			...
			add_action( 'gdm_process_download_request', array( $this, 'check_download_process' ), 5, 2 );
			add_action( 'gdm_sf_process_download_request', array( $this, 'check_download_process' ), 5, 2 );
	...
	public function update_eMember_integration_settings() {
		$settings = get_option( 'gdm_global_options', array() );
	...
	public function show_eMember_integration_settings() {
		$settings = get_option( 'gdm_global_options', array() );
		...
		$output = '<tr>';
		$output .= '<th scope="row">' . __( 'Enable eMember Access Control', 'gluon-download-manager' ) . '</th>';
		$output .= '<td>';
