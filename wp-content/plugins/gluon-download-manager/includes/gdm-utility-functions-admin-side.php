...
function gdm_export_download_logs_to_csv( $start_date, $end_date ) {
	...
	$csv_file_name = sprintf( 'gdm-download-logs-%s_%s.csv', $start_date, $end_date );
...
function gdm_get_downloads_by_date( $start_date = '', $end_date = '', $returnStr = true ) {
	global $wpdb;

	$q = $wpdb->prepare(
		"SELECT COUNT(id) as cnt, DATE_FORMAT(`date_time`,'%%Y-%%m-%%d') as day
            FROM " . $wpdb->prefix . "gdm_downloads
...
function gdm_get_downloads_by_country( $start_date = '', $end_date = '', $returnStr = true ) {
	global $wpdb;
 ...
 * @return array
 */
function gdm_get_all_download_user_agent( $start_date = '', $end_date = '' ) {
	global $wpdb;
...
function gdm_get_all_downloads_by_browser( $start_date = '', $end_date = '' ) {
	$user_agents = gdm_get_all_download_user_agent( $start_date, $end_date );
	...
	return gdm_moveArrayElementToEnd( $browsers, 'Other' );
}
...
function gdm_get_all_downloads_by_os( $start_date = '', $end_date = '' ) {
	$user_agents = gdm_get_all_download_user_agent( $start_date, $end_date );
	...
	return gdm_moveArrayElementToEnd( $operating_systems, 'Other' );
}
 ...
 * @return array
 */
function gdm_get_top_users_by_download_count( $start_date = '', $end_date = '', $limit = 25 ) {
	global $wpdb;
 ...
 * @return array
 */
function gdm_get_top_downloads_by_count( $start_date = '', $end_date = '', $limit = 25 ) {
	global $wpdb;
 ...
 * @return boolean
 */
function gdm_validate_date_field( $data ) {
	if ( is_array( $data ) ) {
 ...
 * @return array
 */
function gdm_moveArrayElementToEnd( array &$array, $key ) {
